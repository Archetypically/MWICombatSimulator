...
import {
    ...
    openableLootDropMap,
    achievementTierDetailMap as achievementTierMap,
...
import HouseRoom from "../src/combatsimulator/houseRoom.js";
...
function initAchievementsModal() {
    ...
    let tierMap = Object.values(achievementTierMap).sort((a, b) => a.sortIndex - b.sortIndex);
    for (const tier of Object.values(tierMap)) {
        let detailMap = Object.values(achievementDetailMap)
            .filter((detail) => detail.tierHrid == tier.hrid)
            .sort((a, b) => a.sortIndex - b.sortIndex);
        ...
        buffValue.innerText =
            ":+" +
            parseFloat(tier["buff"].ratioBoost == 0 ? tier["buff"].flatBoost : tier["buff"].ratioBoost) * 100 +
            "%";
...
function initZones() {
    ...
    let gameZones = Object.values(actionDetailMap)
        .filter(
            (action) =>
                action.type == "/action_types/combat" && action.category != "/action_categories/combat/dungeons",
        )
        .sort((a, b) => a.sortIndex - b.sortIndex);
...
function createDamageDoneAccordion(enemyIndex) {
    const accordionDiv = createElement(
        "div",
        "row d-none",
        "",
        `simulationResultDamageDoneAccordionEnemy${enemyIndex}`,
    );
...
function createDamageTakenAccordion(enemyIndex) {
    const accordionDiv = createElement(
        ...
        "row d-none",
        "",
        `simulationResultDamageTakenAccordionEnemy${enemyIndex}`,
    );
...
function initDamageDoneTaken() {
    for (let i = 64; i > 0; i--) {
        document
            .getElementById("simulationResultTotalDamageDone")
            .insertAdjacentElement("afterend", createDamageDoneAccordion(i));
        document
            .getElementById("simulationResultTotalDamageTaken")
            .insertAdjacentElement("afterend", createDamageTakenAccordion(i));
    }
}
...
function showSimulationResult(simResult) {
    ...
    expensesModalTable.innerHTML =
        '<th data-i18n=\"marketplacePanel.item\">Item</th><th data-i18n=\"marketplacePanel.price\">Price</th><th data-i18n=\"common:amount\">Amount</th><th data-i18n=\"common:total\">Total</th>';
    let revenueModalTable = document.querySelector("#revenueTable > tbody");
    revenueModalTable.innerHTML =
        '<th data-i18n=\"marketplacePanel.item\">Item</th><th data-i18n=\"marketplacePanel.price\">Price</th><th data-i18n=\"common:amount\">Amount</th><th data-i18n=\"common:total\">Total</th>';
    let noRngRevenueModalTable = document.querySelector("#noRngRevenueTable > tbody");
    noRngRevenueModalTable.innerHTML =
        '<th data-i18n=\"marketplacePanel.item\">Item</th><th data-i18n=\"marketplacePanel.price\">Price</th><th data-i18n=\"common:amount\">Amount</th><th data-i18n=\"common:total\">Total</th>';
...
function renderCombatCharts(simResult) {
    ...
    if (
        !simResult.timeSeriesData ||
        !simResult.timeSeriesData.timestamps ||
        simResult.timeSeriesData.timestamps.length === 0
    ) {
        // 显示空状态
        showEmptyCharts();
        return;
    ...
    combatCharts.hpChart = new Chart(document.getElementById("hpChart"), {
        ...
        options: getChartOptions(
            i18next.t("common:Experiment.hpOverTime"),
            i18next.t("common:Experiment.timeInSeconds"),
            "HP",
        ),
    });
    ...
    combatCharts.mpChart = new Chart(document.getElementById("mpChart"), {
        ...
        options: getChartOptions(
            i18next.t("common:Experiment.mpOverTime"),
            i18next.t("common:Experiment.timeInSeconds"),
            "MP",
        ),
    });
    ...
    for (let i = 0; i < simResults.length; i++) {
        for (let j = 0; j < selectedPlayers.length; j++) {
            ...
            if (simResult.experienceGained[playerToDisplay]) {
                totalExperience = Object.values(simResult.experienceGained[playerToDisplay]).reduce(
                    (prev, cur) => prev + cur,
                    0,
                );
            }
            ...
            let displaySimRow = {
                ...
                Encounters: encountersPerHour,
                Deaths: deathsPerHour,
                TotalExperience: totalExperiencePerHour,
                Stamina: experiencePerHour["Stamina"],
                Intelligence: experiencePerHour["Intelligence"],
                Attack: experiencePerHour["Attack"],
                Magic: experiencePerHour["Magic"],
                Ranged: experiencePerHour["Ranged"],
                Melee: experiencePerHour["Melee"],
                Defense: experiencePerHour["Defense"],
        ...
        if (combatMonsterDetailMap()[monster].dropTable) {
            for (const drop of combatMonsterDetailMap()[monster].dropTable) {
                ...
                let dropRate = Math.min(
                    1.0,
                    multiplier * (drop.dropRate + (drop.dropRatePerDifficultyTier ?? 0) * simResult.difficultyTier),
                );
...
function getDropProfit(simResult, playerToDisplay) {
    ...
    simResult["noRngRevenue"] = noRngTotal.toLocaleString("en-US", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
    });
    simResult["expenses"] = expenses.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    simResult["noRngProfit"] = (noRngTotal - expenses).toLocaleString("en-US", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
    });
}
...
function showKills(simResult, playerToDisplay) {
    ...
    if (simResult.isDungeon) {
        let wavesCompletedRow = createRow(
            ["col-md-6", "col-md-6 text-end"],
            ["Max Wave Reached", simResult.maxWaveReached],
        );
        ...
        let completedDungeonsRow = createRow(
            ["col-md-6", "col-md-6 text-end"],
            ["Completed Dungeons", simResult.dungeonsCompleted],
        );
        ...
        if (simResult.dungeonsFailed > 0) {
            let failedDungeonsRow = createRow(
                ["col-md-6", "col-md-6 text-end"],
                ["Failed Dungeons", simResult.dungeonsFailed],
            );
        ...
        let dungeonHoursSimulated =
            simResult.lastDungeonFinishTime > 0 ? simResult.lastDungeonFinishTime / ONE_HOUR : hoursSimulated;
        ...
        let encounterHoursSimulated =
            simResult.lastEncounterFinishTime > 0 ? simResult.lastEncounterFinishTime / ONE_HOUR : hoursSimulated;
    ...
    if (simResult.debuffOnLevelGap[playerToDisplay] != 0) {
        let debuffOnLevelGapRow = createRow(
            ["col-md-6", "col-md-6 text-end"],
            ["Debuff on Level Gap", (simResult.debuffOnLevelGap[playerToDisplay] * 100).toFixed(1) + "%"],
        );
    ...
    let { totalDropMap, noRngTotalDropMap } = !simResult.isDungeon
        ? calcDropMaps(simResult, playerToDisplay)
        : { totalDropMap: new Map(), noRngTotalDropMap: new Map() };
...
function showManapointsGained(simResult, playerToDisplay) {
    ...
    if (simResult.playerRanOutOfMana[playerToDisplay]) {
        ...
        let totalTimeForOut =
            ranOutOfManaStat.totalTimeForOutOfMana +
            (ranOutOfManaStat.isOutOfMana ? simResult.simulatedTime - ranOutOfManaStat.startTimeForOutOfMana : 0);

        let ranOutOfManaStatRow = createRow(
            ["col-md-6", "col-md-6 text-end"],
            ["Run Out Ratio", ((totalTimeForOut / simResult.simulatedTime) * 100).toFixed(2) + "%"],
        );
...
function showDamageDone(simResult, playerToDisplay) {
    ...
    for (const [target, abilities] of Object.entries(simResult.attacks[playerToDisplay])) {
        ...
        resultAccordionButton.innerHTML =
            '<b><span data-i18n="common:simulationResults.damageDone">Damage Done</span> (' +
            '<span data-i18n="monsterNames.' +
            target +
            '">' +
            targetName +
            "</span>" +
    ...
    if (simResult.isDungeon) {
        ...
        for (const waveName of simResult.bossSpawns) {
            ...
            let aliveSecondsSimulated =
                simResult.timeSpentAlive[idx].timeSpentAlive / ONE_SECOND / simResult.timeSpentAlive[idx].count;
            let bossRow = createRow(
                ["col-md-6", "col-md-2", "col-md-4 text-end"],
                [waveNumber, simResult.timeSpentAlive[idx].count, aliveSecondsSimulated.toFixed(1) + "s"],
            );
...
function showDamageTaken(simResult, playerToDisplay) {
    ...
    for (const [source, targets] of Object.entries(simResult.attacks)) {
        ...
        resultAccordionButton.innerHTML =
            '<b><span data-i18n="common:simulationResults.damageTaken">Damage Taken</span> (' +
            '<span data-i18n="monsterNames.' +
            source +
            '">' +
            sourceName +
        ...
        if (maxPlayerCombatLevel / player.combatLevel > 1.2) {
            ...
            console.log(
                "player " +
                    player.hrid +
                    " debuff on level gap: " +
                    player.debuffOnLevelGap * 100 +
                    "% for " +
                    maxPlayerCombatLevel / player.combatLevel,
            );
        } else {
...
function parsePlayerJson(playerJson, hrid) {
    ...
    [
        ...
        let currentEquipment = playerJson.player.equipment.find(
            (item) => item.itemLocationHrid === "/item_locations/" + type,
        );
        if (currentEquipment) {
            playerData.equipment[`/equipment_types/${type}`] = new Equipment(
                currentEquipment.itemHrid,
    ...
    reader.onload = function (event) {
        ...
        try {
            ...
            for (const key in simDataList) {
                ...
                const playersToSim = Object.values(jsonData.players).map((player, index) =>
                    parsePlayerJson(player, `player${index + 1}`),
                );
...
function doGroupExport() {
    try {
        navigator.clipboard
            .writeText(JSON.stringify(playerDataMap))
            .then(() => alert("Current Group has been copied to clipboard."));
    } catch (err) {
...
function doSoloImport() {
    ...
    ["head", "body", "legs", "feet", "hands", "off_hand", "pouch", "neck", "earrings", "ring", "back", "charm"].forEach(
        (type) => {
            let equipmentSelect = document.getElementById("selectEquipment_" + type);
            let enhancementLevelInput = document.getElementById("inputEquipmentEnhancementLevel_" + type);
...
async function fetchPrices() {
    ...
    try {
        ...
        window.prices["/items/small_treasure_chest"] = {
            ask: openableLootDropMap()
                ["/items/small_treasure_chest"].map((item) => {
                    return item.itemHrid in window.prices
                        ? (window.prices[item.itemHrid].ask * item.dropRate * (item.maxCount + item.minCount)) / 2
                        : 0;
                })
                .reduce((a, b) => a + b, 0),
            bid: openableLootDropMap()
                ["/items/small_treasure_chest"].map((item) => {
                    return item.itemHrid in window.prices
                        ? (window.prices[item.itemHrid].bid * item.dropRate * (item.maxCount + item.minCount)) / 2
                        : 0;
                })
                .reduce((a, b) => a + b, 0),
            vendor: openableLootDropMap()
                ["/items/small_treasure_chest"].map((item) => {
        ...
        window.prices["/items/medium_treasure_chest"] = {
            ask: openableLootDropMap()
                ["/items/medium_treasure_chest"].map((item) => {
                    return item.itemHrid in window.prices
                        ? (window.prices[item.itemHrid].ask * item.dropRate * (item.maxCount + item.minCount)) / 2
                        : 0;
                })
        ...
        window.prices["/items/large_treasure_chest"] = {
            ask: openableLootDropMap()
                ["/items/large_treasure_chest"].map((item) => {
                    return item.itemHrid in window.prices
                        ? (window.prices[item.itemHrid].ask * item.dropRate * (item.maxCount + item.minCount)) / 2
                        : 0;
                })
...
document.addEventListener("input", (e) => {
    ...
    if (element.tagName == "TD" && element.parentNode.parentNode.parentNode.classList.value.includes("profit-table")) {
        ...
        if (tableId == "expensesTable") {
            ...
            if (window.prices) {
                if (!window.prices[item])
                    window.prices[item] = { ask: -1, bid: -1, vendor: itemDetailMap()[item].sellPrice };
                if (expensesSetting == "bid") {
